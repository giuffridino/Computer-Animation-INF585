...
void scene_structure::new_water_droplet()
{
	...
	//Initial time
	droplet[2] = timer.t + droplet_delay;
...
void scene_structure::display_frame()
{
	...
	for(int k=0; k<DROPLET_NUMBER; ++k) {
		...
		if (droplet_param[k][3])
		{
			if (1.0f - 1.5 * (timer.t + droplet_delay - droplet_param[k][2]) >= -0.1f)
			{
				falling_sphere.model.translation = vec3(droplet_param[k][0], droplet_param[k][1], 1.0f - 1.5 * (timer.t + droplet_delay - droplet_param[k][2]));
