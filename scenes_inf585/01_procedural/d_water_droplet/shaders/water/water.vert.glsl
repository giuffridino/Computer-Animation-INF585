...
float deformation(float x, float y, float t) {
	...
	for(int k=0; k<DROPLET_NUMBER; k++) {
		float active_t = t - droplet_t0[k];
		float dist = length(vec2(x,y)-vec2(droplet_x0[k],droplet_y0[k])) - radius * active_t;
		// float dist = length(vec2(x,y)-vec2(droplet_x0[k],droplet_y0[k])) - radius * active_t;
		if(active_t >= animTime) 
		{
			droplet_active[k] = 0.0;
		}
		else if(droplet_active[k]==1.0) {
			z_out += bumpIntensity * exp(-((dist - radius * active_t) * (dist - radius * active_t))/(2 * std * std)) * cos(( 0.5 * 3.14159 / animTime) * active_t) + bumpIntensity * exp(-((dist - (radius * (active_t + 1.0f))) * (dist - (radius * (active_t + 1.0f))))/(2 * std * std)) * cos(( 0.5 * 3.14159 / animTime) * (active_t + 1.0f));
		}
	}
