...
uniform int is_environment_map;
uniform int is_fog;
...
void main()
{
	...
	if(is_environment_map==0) {
		color_environment_map = vec4(1.0);
	}
	...
	float dist_to_camera = length(camera_position-fragment.position.xyz);
	float alpha_fog = min(dist_to_camera/10.0,1.0);
	alpha_fog = alpha_fog*alpha_fog;
	if(is_fog==0) {
		alpha_fog = 0.0;
	}
	color_shading = (1.0-alpha_fog)*color_shading+alpha_fog*background_color;
