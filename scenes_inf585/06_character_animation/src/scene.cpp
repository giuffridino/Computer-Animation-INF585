...
void scene_structure::initialize()
{
	...
	// Initializing the effect walking for better position of the character
}
...
void scene_structure::display_gui()
{
	...
	if(is_walk_clicked && effect_walk.active){
		...
		std::cout << "Resetting root_position in start of walk\n";
		// Don't set it to a specific value but add on it depending on which character is active (future things to do)
		effect_walk.root_position = vec3(0,1.1f,0);
	}
	...
	for(auto& entry : characters) {
		...
		if( ImGui::Button(name.c_str()) ) {
			...
			if(effect_walk.active) {
				effect_transition[current_active_character].transition_time = 0.2f;
				characters[current_active_character].set_current_animation("Idle");
				std::cout << "Resetting root_position\n";
