...
float kernel(float val, float power_factor)
{
	return (1.0f / (pow(val, power_factor)));
}

float distanceToSegment(vec3 point, vec3 segmentStart, vec3 segmentEnd)
{
	vec3 seg = segmentEnd - segmentStart;
	vec3 segStartPoint = point - segmentStart;
	float dotProd = dot(seg, segStartPoint);
	float t = dotProd / (seg.x * seg.x + seg.y * seg.y + seg.z * seg.z);
	if (t < 0.0f)
		return norm(segStartPoint);
	else if (t > 1.0f)
	{
		float deltaX = point.x - segmentEnd.x;
		float deltaY = point.y - segmentEnd.y;
        float deltaZ = point.z - segmentEnd.z;
		return norm(vec3(deltaX, deltaY, deltaZ));
	}
	vec3 closestPoint = vec3(segmentStart.x + t * seg.x, segmentStart.y + t * seg.y, segmentStart.z + t * seg.z);
	return norm(vec3(point - closestPoint));
}
...
numarray<numarray<float> > compute_skinning_weights_generic(cgp::mesh const& m, skeleton_structure const& skeleton, float power_factor) 
{
	...
	for(int k_vertex=0; k_vertex<N_vertex; ++k_vertex) 
	{
		vec3 vertex = m.position[k_vertex];
		float sum = 0.0;
		for (int j_joint = 0; j_joint < N_joint; j_joint++)
		{
			vec3 parent = skeleton.joint_matrix_global_bind_pose[j_joint].get_block_translation();
			float min_dist = INFINITY;
			for (auto c_child : skeleton.child(j_joint))
			{
				vec3 child = skeleton.joint_matrix_global_bind_pose[c_child].get_block_translation();
				float dist = distanceToSegment(vertex, parent, child);
				if (dist < min_dist)
					min_dist = dist;
			}
			...
			{
				weights[k_vertex][j_joint] = 1/pow(min_dist,power_factor);
				sum += 1/pow(min_dist,power_factor);
			}
			else
			{
				weights[k_vertex][j_joint] = 0.0;
			}
		}

		for(int j_joint = 0; j_joint < N_joint; j_joint++)
		{
			weights[k_vertex][j_joint] /= sum;
		}
	}
	return weights;
}
